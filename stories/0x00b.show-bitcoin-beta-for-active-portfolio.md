...
- Missing/stale prices: assets without enough history in the lookback window are
  ...
  weights). The API returns `data_age_hours`, and the UI warns when the
  underlying daily candle data is older than 24 hours. The UI must indicate when
  renormalization occurred and which assets were excluded.

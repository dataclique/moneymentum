...
const transformToLineData = (data: TradingData[], metric: string): { time: string; value: number }[] => {
...
const ChartComponent: React.FC<ChartComponentProps> = ({ data, selectedMetric, height = 600 }) => {
    ...
    try {
      ...
      if (selectedMetric === 'price') {
        ...
        if (range < 0.0000001 && avgAbsValue < 0.000001) {
          precision = 8;
        } else if (selectedMetric.includes('return') || selectedMetric === 'sharpe' || selectedMetric === 'sortino') {
        ...
        } else if (selectedMetric.includes('volatility') || selectedMetric.includes('stddev')) {
        ...
        console.log(`Metric: ${selectedMetric}, Value range: ${minValue.toFixed(8)} to ${maxValue.toFixed(8)}, avg abs: ${avgAbsValue.toFixed(8)}, using precision: ${precision}`);
...
const TokenPage: React.FC = () => {
    ...
    React.useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const response = await fetch(`http://localhost:8000/api/token/${ticker}`);
                if (!response.ok) {
    ...
    const selectedMetricLabel = React.useMemo(() => {
        return AVAILABLE_METRICS.find(m => m.value === selectedMetric)?.label || selectedMetric;
    }, [selectedMetric]);

    const handleMetricChange = React.useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {

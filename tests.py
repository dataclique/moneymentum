...
def test_aave_last_record(spark_session):
    ...
    # Google sheet: 0.29%
    assert aave_last_record["mean_return"] == pytest.approx(
        0.0029290822331748565
    ), "Mean return mismatch"
    # Google sheet: 0.05
    assert aave_last_record["return_stddev"] == pytest.approx(
        0.05116662705011916
    ), "Return stddev mismatch"
    # Google sheet: 97.76%
    assert aave_last_record["annualized_volatility"] == pytest.approx(
        0.9775370372243625
    ), "Annualized volatility mismatch"
    # Google sheet: 0.29
    assert aave_last_record["beta"] == pytest.approx(0.29206334291840363), "Beta mismatch"
    # Google sheet: 7.67E-04
    assert aave_last_record["covariance"] == pytest.approx(
        7.646287605794159e-4
    ), "Covariance mismatch"
...
def test_btc_last_record(spark_session):
    ...
    # Google sheet: 0.22%
    assert btc_last_record["mean_return"] == pytest.approx(
        0.002159416213018761
    ), "Mean return mismatch"
    # Google sheet: 0.03
    assert btc_last_record["return_stddev"] == pytest.approx(
        0.027437960505078306
    ), "Return stddev mismatch"
    # Google sheet: 52.42%
    assert btc_last_record["annualized_volatility"] == pytest.approx(
        0.5242014994136859
    ), "Annualized volatility mismatch"
    # Google sheet: 1.00
    assert btc_last_record["beta"] == pytest.approx(0.9972899728997291), "Beta mismatch"
    # Google sheet: 7.53E-04
    assert btc_last_record["covariance"] == pytest.approx(
        7.508014553322255e-4
    ), "Covariance mismatch"
...
def test_ai_last_record(spark_session):
    ...
    # Google sheet: -1.27%
    assert ai_last_record["mean_return"] == pytest.approx(
        -0.012734350383113274
    ), "Mean return mismatch"
    # Google sheet: 0.16
    assert ai_last_record["return_stddev"] == pytest.approx(
        0.16124563962681307
    ), "Return stddev mismatch"
    # Google sheet: 116.28%
    assert ai_last_record["annualized_volatility"] == pytest.approx(
        1.1627588432389253
    ), "Annualized volatility mismatch"
    # Google sheet: 0.28
    assert ai_last_record["beta"] == pytest.approx(0.27410619693561644), "Beta mismatch"
    # Google sheet: 7.27E-03
    assert ai_last_record["covariance"] == pytest.approx(
        0.007126803962757325
    ), "Covariance mismatch"
